// 本地演示数据服务：用于答辩、截图和离线体验，不依赖已失效的旧数据库。
// 正式接入可用数据库后，将 DEMO_MODE 改为 false 即可恢复真实云函数调用。
...
function install() {
  if (!DEMO_MODE || installed || !wx.cloud) return;
  installed = true;
  const originalCallFunction = wx.cloud.callFunction;
  wx.cloud.callFunction = options => new Promise(resolve => {
  ...
  wx.cloud.uploadFile = options => Promise.resolve({ fileID: (options && options.filePath) || covers.algorithm });
  console.log('[DemoService] 演示数据模式已启用，真实云函数调用已在当前设备拦截。', originalCallFunction ? '' : '');
}

module.exports = { isEnabled: () => DEMO_MODE, install };
